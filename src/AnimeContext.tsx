    ...
    const dataToSave = [
      {
        ...
        lista_animes: lists.porVer,
      },
      {
        id_lista: 2,
        id_utilizador: user.id_utilizador,
        lista_animes: lists.aVer,
      },
      {
        id_lista: 3,
        id_utilizador: user.id_utilizador,
        lista_animes: lists.completado,
      },
    ];
    ...
    try {
      for (const data of dataToSave) {
        ...
        if (!checkResponse.ok) {
          ...
          console.error(
            `Erro ao verificar a lista: ${checkResponse.statusText} - ${errorText}`
          );
          throw new Error(
            `Erro ao verificar a lista: ${checkResponse.statusText} - ${errorText}`
          );
        ...
        if (!response.ok) {
          ...
          console.error(
            `Erro ao salvar a lista: ${response.statusText} - ${errorText}`
          );
          throw new Error(
            `Erro ao salvar a lista: ${response.statusText} - ${errorText}`
          );
